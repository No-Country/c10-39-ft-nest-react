...
const HoursList: FC<{ handleClick: () => void; getAllHours: boolean }> = ({
  ...
  return (
    <>
      {hours.length ? (
        <div className="bg-white absolute top-32 -left-24 lg:top-0 lg:-left-full py-5 rounded-md shadow-lg">
          <span className="mb-2 divide-black border-b-[1px] mx-2 px-1 text-center block w-[90%]">
            {getAllHours ? 'Horarios' : 'Horarios disponibles'}
          </span>
          <ul className="overflow-y-scroll max-h-[160px] relative left-1">
            {hours.map((item) => {
              return (
                <li
                  key={item.id}
