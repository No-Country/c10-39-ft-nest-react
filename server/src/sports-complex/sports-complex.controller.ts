...
export class SportsComplexController {
  ...
  async create(
    ...
    if (owner) throw new NotFoundException("User is owner");

    //primero cremos el owner
    const newOwner: Owner = await this.sportsComplexService.createOwner({
      ...createSportsComplexDto,
      user,
    });
    //luego creamos el sports complex
    console.log(createSportsComplexDto);
    

    return this.sportsComplexService.create(
      createSportsComplexDto,
      newOwner
    );
  }
